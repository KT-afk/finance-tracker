...
async function main() {
  if (!fixturePath || !existsSync(fixturePath)) {
    console.log(
      "UOB upload confirm flow test skipped: set UOB_MAY_PDF to a real UOB May PDF"
    )
    return
  }
  ...
  const { transactions: parsedTxs } = parsed
  assert.equal(parsedTxs.length, 45)
  assert.equal(parsedTxs.filter((t) => t.date.startsWith("2026-05")).length, 33)
  ...
  formData.set(
    "file",
    new File([readFileSync(fixturePath)], path.basename(fixturePath), {
      type: "application/pdf",
    })
  )

  const previewRes = await previewUpload(
  ...
  const mayRes = await getTransactions(
    new Request(
  ...
  assert.ok(
    mayBody.transactions.some((tx: { description: string }) =>
      tx.description.includes("CLAUDE.AI SUBSCRIPTION")
    ),
