...
if [ -z "${APP_PASSWORD:-}" ]; then
  echo "APP_PASSWORD is not set. Add it to .env.local before verifying phone edits."
  exit 1
fi
...
echo "Verifying reversible phone edit at $FINANCE_TRACKER_URL"
exec bash "$PROJECT_DIR/scripts/verify-hosted-edit.sh"
