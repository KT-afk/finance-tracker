...
function isBusinessName(name: string): boolean {
  ...
  const businessSuffixes = ['PTE', 'LTD', 'SDN', 'BHD', 'STUDIO', 'INC', 'CORP', 'CO.', 'PLC']
...
function extractMemoKeyword(memo: string): string {
  const stopwords = new Set(['for', 'the', 'and', 'to', 'a', 'an', 'of', 'in', 'on', 'at'])
  const words = memo
    .toLowerCase()
    .split(/\s+/)
...
export async function categorize(description: string): Promise<Category> {
  ...
  try {
    const prompt = buildPrompt(description, memo, recipient, method)
