...
export default function HomePage() {
  ...
  const savingsBalanceCount =
    balanceData?.balances.filter(
      (balance) => balance.account_type !== "credit_card"
    ).length ?? 0
  ...
  return (
    <div className="space-y-6 p-4 max-w-2xl mx-auto">
        ...
        <div className="space-y-5">
          {[1, 2, 3].map((i) => (
            <div
              ...
              style={{ height: i === 1 ? "80px" : i === 2 ? "140px" : "180px" }}
            />
          ))}
        </div>
      )}
      ...
      {/* Keep saved account balances available even when the selected month has no transactions. */}
      {isEmpty && balanceData && balanceData.balances.length > 0 && (
        <Card className="bg-zinc-900 border-zinc-800">
          ...
          <CardContent>
            <Link
              href="/accounts"
              className="block rounded-lg bg-zinc-800/50 p-3 hover:bg-zinc-800 transition-colors"
            >
              <p className="text-2xl font-mono font-semibold text-white">
                {formatSGD(balanceData.total)}
              </p>
              <p className="mt-1 text-xs text-zinc-500">
                {savingsBalanceCount} savings account
                {savingsBalanceCount === 1 ? "" : "s"}; updated{" "}
                {relativeDate(balanceData.lastUpdated)}
              </p>
            </Link>
          </CardContent>
        </Card>
      ...
      {data && !isEmpty && (
        <>
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardHeader>
              <CardTitle className="text-sm text-zinc-400 font-normal">
                {getMonthLabel(selectedMonth)} Financial Health
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
                  ...
                  <div className="text-center p-3 rounded-lg bg-zinc-800/50 hover:bg-zinc-800 transition-colors cursor-pointer">
                    ...
                    {balanceData && balanceData.balances.length > 0 && (
                      <p className="text-xs text-zinc-500 mt-1">
                        {balanceData.balances.length} bank
                        {balanceData.balances.length > 1 ? "s" : ""}
                      </p>
                    )}
                  </div>
                </Link>
                ...
                data.priorMonthLabel && (
                  <div className="flex items-center justify-between p-3 rounded-lg bg-zinc-800/30">
                    ...
                    <div className="text-right">
                      ...
                      >
                        {data.momDelta < 0 ? "↓" : "↑"}{" "}
                        {formatSGD(Math.abs(data.momDelta))}
                      </p>
                      <p
                ...
                <div className="p-3 rounded-lg bg-zinc-800/30">
                  ...
                  <div className="space-y-2">
                    {balanceData.balances
                      .filter((b) => b.account_type !== "credit_card")
                      .map(({ bank, balance }) => (
                        <div
                          key={bank}
              ...
              <div className="p-3 rounded-lg bg-zinc-800/30">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-zinc-400">
                    Day {data.daysElapsed} of{" "}
                    {new Date(data.month + "-01").getDate() === 1
                ...
                <div className="w-full bg-zinc-700 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${Math.min((data.daysElapsed / 30) * 100, 100)}%`,
                    }}
                  />
                </div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className="text-sm text-zinc-200 group-hover:text-white transition-colors duration-200">
                            {cat.category}
                          </span>
                        </div>
