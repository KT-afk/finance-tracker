...
async function buildTransactionSummary(): Promise<string> {
  ...
  const monthMap: Record<
    string,
    Record<string, { total: number; merchants: Record<string, number> }>
  > = {}
  for (const tx of rows) {
    if (tx.amount >= 0) continue // skip income/credits
    const month = tx.date.slice(0, 7) // YYYY-MM
    if (!monthMap[month]) monthMap[month] = {}
    if (!monthMap[month][tx.category])
      monthMap[month][tx.category] = { total: 0, merchants: {} }
    monthMap[month][tx.category].total += Math.abs(tx.amount)
    monthMap[month][tx.category].merchants[tx.description] =
      (monthMap[month][tx.category].merchants[tx.description] ?? 0) +
      Math.abs(tx.amount)
  }
  ...
  return months
    .map((month) => {
      const catEntries = Object.entries(monthMap[month]).sort(
        ([, a], [, b]) => b.total - a.total
      )
...
- Valid categories: Food & Drink, Groceries, Transport, Shopping, Subscriptions, Health, Entertainment, Bills & Utilities, Housing, Transfer, Self Transfer, Reimbursement, Credit Card Payment, Personal, Income, Others`
...
${question}`
    ...
    if (parsed.intent === "correction" && parsed.correction) {
      ...
      if (
        ...
        const matching = await db
          ...
          .from(transactions)
          .where(
            sql`lower(${transactions.description}) like lower(${"%" + descriptionFragment + "%"})`
          )
    ...
    await db.insert(aiConversations).values({
      ...
      answer_data: parsed.answer_data
        ? JSON.stringify(parsed.answer_data)
        : null,
      created_at: new Date().toISOString(),
    })
